...
import Grease.LLVM.Overrides.Diagnostic as Diag (Diagnostic (CantSkip, FoundDeclare, RegisteredOverride), ppType)
...
import Lang.Crucible.LLVM.DataLayout qualified as CLI
import Lang.Crucible.LLVM.Functions qualified as CLI
...
-- | Bind a 'C.FnHandle' to an 'CLI.LLVMOverride'.
bindLLVMOverrideFnHandle ::
...
bindLLVMOverrideFnHandle mvar hdl llOv = do
  ...
  let llArgs = CLI.llvmOvArgs llOv
  let llRet = CLI.llvmOvRet llOv
  case (C.testEquality llArgs hdlArgs, C.testEquality llRet hdlRet) of
    (Just C.Refl, Just C.Refl) -> do
      let typedOv = CLI.llvmOverrideToTypedOverride mvar llOv
...
registerLLVMOverrides la builtinOvs userOvs skipFuns bak llvmCtx fs defns decls errCb = do
  ...
  Foldable.forM_ allDecls $ \(Decl.SomeDeclare decl) -> do
    ...
    CS.modifyGlobal mvar $ \mem ->
      case Map.lookup symb (CLM.memImplGlobalMap mem) of
        ...
        _ -> liftIO (CLI.registerFunPtr bak mem name symb aliases)
  ...
  builtinBasicDeclOvs <-
    CLI.register_llvm_overrides_ llvmCtx builtinOvsList basicDecls
  builtinFwdDeclOvs <-
  ...
  builtinDeclOvs <-
    CLI.register_llvm_overrides_ llvmCtx ovs decls
  let builtinOvs' =
...
-- override names to their corresponding 'CLI.SomeLLVMOverride's, suitable
