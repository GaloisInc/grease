...
insertFunctionHandle state handle fnState =
  let C.FnBindings curHandles = state ^. C.stateContext ^. C.functionBindings in
  let newHandles = C.FnBindings $
                   C.insertHandleMap handle fnState curHandles in
  set (C.stateContext . C.functionBindings) newHandles state
