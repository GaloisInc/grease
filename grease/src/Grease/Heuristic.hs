...
type RefineHeuristic sym bak ext tys precond =
  ...
  precond ->
  IO (HeuristicResult ext tys precond)
...
newtype MemoryErrorHeuristic sym ext w argTys precond
  = MemoryErrorHeuristic
      ( forall ts t.
        ...
        IO (HeuristicResult ext argTys precond)
      )
...
pointerHeuristic la ptrHeuristic byteHeuristic bak anns initMem obligation minfo argNames args =
 ...
 where
  ...
  applyToErr :: AnyMemError sym -> CLMP.LLVMPtr sym w -> IO (HeuristicResult ext argTys precond)
...
llvmHeuristics ::
  ...
  , precond ~ ArgShapes LLVM NoTag argTys
  ) =>
  GreaseLogAction ->
...
macawHeuristics ::
  ...
  , CLM.HasLLVMAnn sym
  , ?memOpts :: CLM.MemOptions
  , precond ~ ArgShapes (Symbolic.MacawExt arch) NoTag (Symbolic.MacawCrucibleRegTypes arch)
  ) =>
  GreaseLogAction ->
  RegNames arch ->
...
macawHeuristics la rNames =
 ...
 where
  handleMemErr' ::
    ...
    IO (HeuristicResult (Symbolic.MacawExt arch) (Symbolic.MacawCrucibleRegTypes arch) precond)
