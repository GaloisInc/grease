...
type ScreachSimulatorState ::
  Type -> Type -> Type -> Type -> Type -> Type -> Type -> C.Ctx CT.CrucibleType -> Type
data ScreachSimulatorState p sym bak ext rtp arch t aty = ScreachSimulatorState
  { _greaseSimulatorState :: GMSS.GreaseSimulatorState MDebug.MacawCommand sym arch
  , _schedulerState ::
      Sched.SchedulerState (ScreachSimulatorState p sym bak ext rtp arch t aty) sym ext rtp
  , _distState :: Dist.DijkstraCaches
  , _recordState ::
      CSR.RecordState (ScreachSimulatorState p sym bak ext rtp arch t aty) sym ext rtp
  , _replayState ::
      CSR.ReplayState (ScreachSimulatorState p sym bak ext rtp arch t aty) sym ext rtp
  , _refineState :: RFT.SrchRefineData sym bak t ext aty
  }

makeLenses ''ScreachSimulatorState

instance ShortDistSched.HasDistancesState (ScreachSimulatorState p sym bak ext rtp arch t aty) where
  distanceState = distState

instance
  Sched.HasSchedulerState
    (ScreachSimulatorState p sym bak ext rtp arch t aty)
    (ScreachSimulatorState p sym bak ext rtp arch t aty)
    sym
    ext
    rtp
...
instance
  ...
  Dbg.HasContext (ScreachSimulatorState p sym bak ext rtp arch t aty) MDebug.MacawCommand sym ext ret
  where
...
instance
  ...
  MS.HasMacawLazySimulatorState (ScreachSimulatorState p sym bak ext rtp arch t aty) sym w
  where
...
instance ToConc.HasToConcretize (ScreachSimulatorState p sym bak ext rtp arch t aty) where
